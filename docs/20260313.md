...
単純にLangSmithにデータを保存できるようにするデコレータということらしいね
...
```
結局このスキーマに従って出力してねというプロンプトベースの手法なので、これはええんやろかというのは気になるな
...
結局ほんとにプロンプトで判断させてるだけなんだけど、これは大丈夫なのかな？
""Please tell a lie about the following user question.
Documents:
{docs_string}"""
試しにこんなプロンプトにしてみた。
![alt text](image-2.png)
まあ一応T/Fの筋は通るけど、割と評価自体は適当なんじゃないかというふうに感じた。
むしろFalseにする方が難しいと思う。
とりあえずまあ、評価器を作って、LangSmith上で結果確認すると楽ちんだねというのはよくわかった。

これ以上LanhChainのDocsおってもRAGの性能向上には寄与しなさそうなので、
次に何をしていくかを考えてみる。
