...
💬 メッセージ履歴 (8 件):
  ...
  [3]         ai: You’re John Smith. If that’s not correct, let me know and I can update it.
  [4]      human: What theme do I prefer?
  [5]         ai:
  [6]       tool: User is John Smith
  [7]         ai: I don’t have your theme preference on file. Do you mean a UI theme (light, dark, or system) or another kind of theme (e....

🏷  user_id: user_123

🏷  preferences: {'theme': 'dark'}
```
`checkpoint_id`は毎回変更されているので、前のメッセージの状態にロールバックとかもできそう
...
# > User is John Smith.
```
こんな感じで`ToolRuntime`を経由してアクセスすることも可能
...
for msg in result["messages"]:
    msg.pretty_print()
```
ミドルウェアの短期メモリ (状態) にアクセスして、会話履歴またはカスタム状態フィールドに基づいて動的なプロンプトを作成することもできる
結局短期記憶の定義
